...
foreach($folders as $x) {
  ...
  @mkdir('../game-nodejs-server/tiles/' . $x);
  foreach($files as $f2) {
    if (!file_exists('../game-nodejs-server/tiles/' . $x . '/' . $f2)) {
      ...
      $result = array(
        ...
        'tile' => $tile,
        'tile_m' => $tile_m,
        'water' => processField(array_filter($data['features'], function ($i) {
          return $i['properties']['vt_layer'] == 'water' || $i['properties']['vt_layer'] == 'waterway';
        }), $tile_m),
        'buildings' => processField(array_filter($data['features'], function ($i) {
          return $i['properties']['vt_layer'] == 'building';
        }), $tile_m),
        'roads' => processField(array_filter($data['features'], function ($i) {
          return $i['properties']['vt_layer'] == 'road';
        }), $tile_m)
      );

      file_put_contents('../game-nodejs-server/tiles/' . $x . '/' . $f2, json_encode($result));
    }
  }
}
